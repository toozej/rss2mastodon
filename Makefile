...
REPO_URL := https://github.com/toozej/rss2mastodon

# Detect the OS and architecture
OS := $(shell uname -s)
ARCH := $(shell uname -m)
LATEST_RELEASE_URL := $(REPO_URL)/releases/latest/download/rss2mastodon_$(OS)_$(ARCH).tar.gz
...
install: ## Install rss2mastodon from latest GitHub release
	if command -v go; then \
			go install github.com/toozej/rss2mastodon@latest ; \
	else \
			echo "Downloading rss2mastodon binary for $(OS)-$(ARCH)..."; \
			mkdir -p $(CURDIR)/tmp; \
			curl --silent -L -o $(CURDIR)/tmp/rss2mastodon.tgz $(LATEST_RELEASE_URL); \
			tar -xzf $(CURDIR)/tmp/rss2mastodon.tgz -C $(CURDIR)/tmp/; \
			chmod +x $(CURDIR)/tmp/rss2mastodon; \
			sudo mv $(CURDIR)/tmp/rss2mastodon /usr/local/bin/rss2mastodon; \
			rm -rf $(CURDIR)/tmp; \
	fi
...
local-kill: ## Kill any currently running locally built binary
	-pkill -f '$(CURDIR)/out/rss2mastodon'
...
profile-cpu: ## Generate CPU performance profile
	...
	go test -bench=. -cpuprofile=$(CURDIR)/profiles/cpu.prof $(CURDIR)/internal/rss2mastodon/
...
profile-mem: ## Generate memory performance profile
	...
	mkdir -p $(CURDIR)/profiles
	go test -bench=. -memprofile=$(CURDIR)/profiles/mem.prof $(CURDIR)/internal/rss2mastodon/
...
benchmark: ## Run benchmarks
	...
	go test -bench=. -benchmem $(CURDIR)/internal/rss2mastodon/
